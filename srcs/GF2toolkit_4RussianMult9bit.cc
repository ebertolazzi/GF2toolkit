...
namespace GF2toolkit {
  ...
  */
  ...
  #define TT(N) B_Tables[N*(1<<9)+BITS9(tmp,N)]
  #define TTT   TT(0)^TT(1)^TT(2)^B_Tables[3*(1<<9)+(tmp>>27)]
  
  template <>
  void
  MM4RmultAssBlock<uint32_t,9>( uint32_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint32_t,9>( uint32_t const * A, unsigned nRowsA,
                                uint32_t const * B_Tables,
                                uint32_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint32_t,9>( uint32_t       * A, unsigned nRowsA,
                                  uint32_t const * B_Tables ) {
  ...
  //////////////////////////////////////////////////////////////////////////////
  #undef  TTT
  #define TTT TT(0)^TT(1)^TT(2)^TT(3)^TT(4)^TT(5)^B_Tables[6*(1<<9)+(tmp>>54)]
  //////////////////////////////////////////////////////////////////////////////

  template <>
  void
  MM4RmultAssBlock<uint64_t,9>( uint64_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint64_t,9>( uint64_t const * A, unsigned nRowsA,
                                uint64_t const * B_Tables,
                                uint64_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint64_t,9>( uint64_t       * A, unsigned nRowsA,
                                  uint64_t const * B_Tables ) {
  ...
  #undef  TT
  #undef  TTT
  #define TT(IDX) tmp = _mm_xor_si128( tmp, TB[IDX*(1<<9)+BITS9(tt,IDX)] )
  #define MM4R_SINGLE_OP_INTERNAL                                              \
  tt = A->i64.lo ;                                                             \
  TB = (__m128i const *)B_Tables ;                                             \
  TT(0); TT(1); TT(2); TT(3); TT(4); TT(5);                                    \
  tmp = _mm_xor_si128( tmp, TB[6*(1<<9)+(tt>>54)] ) ;                          \
  tt  = A->i64.hi ;                                                            \
  TB += 6*(1<<9)+(1<<10) ;                                                     \
  TT(0); TT(1); TT(2); TT(3); TT(4); TT(5);                                    \
  tmp = _mm_xor_si128( tmp, TB[6*(1<<9)+(tt>>54)] )
  ...
  template <>
  void
  MM4RmultAssBlock<uint128_t,9>( uint128_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint128_t,9>( uint128_t const * A, unsigned nRowsA,
                                 uint128_t const * B_Tables,
                                 uint128_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint128_t,9>( uint128_t       * A, unsigned nRowsA,
                                   uint128_t const * B_Tables ) {
...
// EOF GF2toolkit_4RussianMult9bit.cc
