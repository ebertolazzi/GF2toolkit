...
namespace GF2toolkit {
  ...
  */
  ...
  #define TT(N) B_Tables[N*(1<<3)+BITS3(tmp,N)]
  #define TTT   TT(0)^TT(1)^TT(2)^TT(3)^TT(4)^TT(5)^TT(6)^TT(7)^ \
                B_Tables[8*(1<<3)+((tmp>>24)&0x0F)]^ \
                B_Tables[8*(1<<3)+(1<<4)+(tmp>>28)]
  template <>
  void
  MM4RmultAssBlock<uint32_t,3>( uint32_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint32_t,3>( uint32_t const * A, unsigned nRowsA,
                                uint32_t const * B_Tables,
                                uint32_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint32_t,3>( uint32_t       * A, unsigned nRowsA,
                                  uint32_t const * B_Tables ) {
  ...
  #define TTT TT(0) ^TT(1) ^TT(2) ^TT(3) ^TT(4) ^TT(5) ^TT(6) ^TT(7) ^  \
              ...
              TT(16)^TT(17)^TT(18)^TT(19)^B_Tables[20*(1<<3)+(tmp>>60)]
  //////////////////////////////////////////////////////////////////////////////

  template <>
  void
  MM4RmultAssBlock<uint64_t,3>( uint64_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint64_t,3>( uint64_t const * A, unsigned nRowsA,
                                uint64_t const * B_Tables,
                                uint64_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint64_t,3>( uint64_t       * A, unsigned nRowsA,
                                  uint64_t const * B_Tables ) {
  ...
  #undef  TTT
  #undef  TT
  #define TT(IDX) tmp = _mm_xor_si128( tmp, TB[IDX*(1<<3)+BITS3(tt,IDX)] )
  #define MM4R_SINGLE_OP_INTERNAL                                              \
  tt = A->i32.n0 ;                                                             \
  TB = (__m128i const *)B_Tables ;                                             \
  TT(0); TT(1); TT(2); TT(3); TT(4); TT(5); TT(6); TT(7); TT(8); TT(9);        \
  tt = (A->i32.n0>>30)|(A->i32.n1<<2) ;                                        \
  TB += 10*(1<<3) ;                                                            \
  TT(0); TT(1); TT(2); TT(3); TT(4); TT(5); TT(6); TT(7); TT(8); TT(9);        \
  tt = (A->i32.n1>>28)|(A->i32.n2<<4) ;                                        \
  TB += 10*(1<<3) ;                                                            \
  TT(0); TT(1); TT(2); TT(3); TT(4); TT(5); TT(6); TT(7); TT(8); TT(9);        \
  tt = (A->i32.n2>>26)|(A->i32.n3<<6) ;                                        \
  TB += 10*(1<<3) ;                                                            \
  TT(0); TT(1); TT(2); TT(3); TT(4); TT(5); TT(6); TT(7); TT(8); TT(9);        \
  tt = A->i32.n3>>24 ;                                                         \
  TB += 10*(1<<3) ;                                                            \
  tmp = _mm_xor_si128( tmp, TB[tt&0x0F] ) ;                                    \
  tmp = _mm_xor_si128( tmp, TB[(1<<4)+(tt>>4)] )

  template <>
  void
  MM4RmultAssBlock<uint128_t,3>( uint128_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint128_t,3>( uint128_t const * A, unsigned nRowsA,
                                 uint128_t const * B_Tables,
                                 uint128_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint128_t,3>( uint128_t       * A, unsigned nRowsA,
                                   uint128_t const * B_Tables ) {
...
// EOF GF2toolkit_4RussianMult3bit.cc
