...
namespace GF2toolkit {
  ...
  */
  ...
  #define TT(N) B_Tables[N*(1<<5)+BITS5(tmp,N)]
  #define TTT   TT(0)^TT(1)^TT(2)^TT(3)^TT(4)^B_Tables[5*(1<<5)+(tmp>>25)]

  template <>
  void
  MM4RmultAssBlock<uint32_t,5>( uint32_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint32_t,5>( uint32_t const * A, unsigned nRowsA,
                                uint32_t const * B_Tables,
                                uint32_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint32_t,5>( uint32_t       * A, unsigned nRowsA,
                                  uint32_t const * B_Tables ) {
  ...
  #define TTT TT(0)^TT(1)^TT(2)^TT(3)^TT(4)^TT(5)^TT(6)^TT(7)^ \
              TT(8)^TT(9)^TT(10)^TT(11)^B_Tables[12*(1<<5)+(tmp>>60)]
  //////////////////////////////////////////////////////////////////////////////

  template <>
  void
  MM4RmultAssBlock<uint64_t,5>( uint64_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint64_t,5>( uint64_t const * A, unsigned nRowsA,
                                uint64_t const * B_Tables,
                                uint64_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint64_t,5>( uint64_t       * A, unsigned nRowsA,
                                  uint64_t const * B_Tables ) {
  ...
  #undef  TT
  #define TT(IDX) tmp = _mm_xor_si128( tmp, TB[IDX*(1<<5)+BITS5(tt,IDX)] )
  ...
  tt = A->i32.n3>>24 ;                                                         \
  TB += 6*(1<<5) ;                                                             \
  TT(0); tmp = _mm_xor_si128( tmp, TB[(1<<5)+(tt>>5)] )

  template <>
  void
  MM4RmultAssBlock<uint128_t,5>( uint128_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint128_t,5>( uint128_t const * A, unsigned nRowsA,
                                 uint128_t const * B_Tables,
                                 uint128_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint128_t,5>( uint128_t       * A, unsigned nRowsA,
                                   uint128_t const * B_Tables ) {
...
// EOF GF2toolkit_4RussianMult5bit.cc
