...
namespace GF2toolkit {
  ...
  */                   
  ...
  #define TT(N) B_Tables[N*(1<<10)+BITS10(tmp,N)]
  #define TTT   TT(0)^TT(1)^B_Tables[2*(1<<10)+(tmp>>20)]
  
  template <>
  void
  MM4RmultAssBlock<uint32_t,10>( uint32_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint32_t,10>( uint32_t const * A, unsigned nRowsA,
                                 uint32_t const * B_Tables,
                                 uint32_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint32_t,10>( uint32_t       * A, unsigned nRowsA,
                                   uint32_t const * B_Tables ) {
  ...
  //////////////////////////////////////////////////////////////////////////////
  #undef  TTT
  #define TTT TT(0)^TT(1)^TT(2)^TT(3)^TT(4)^TT(5)^B_Tables[6*(1<<10)+(tmp>>60)]
  //////////////////////////////////////////////////////////////////////////////

  template <>
  void
  MM4RmultAssBlock<uint64_t,10>( uint64_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint64_t,10>( uint64_t const * A, unsigned nRowsA,
                                 uint64_t const * B_Tables,
                                 uint64_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint64_t,10>( uint64_t       * A, unsigned nRowsA,
                                   uint64_t const * B_Tables ) {
  ...
  #undef  TT
  #undef  TTT
  #define TT(IDX) tmp = _mm_xor_si128( tmp, TB[IDX*(1<<10)+BITS10(tt,IDX)] )
  ...
  tt = A->i64.hi>>56 ;                                                         \
  TB += 6*(1<<10) ;                                                            \
  tmp = _mm_xor_si128( tmp, TB[tt] )

  template <>
  void
  MM4RmultAssBlock<uint128_t,10>( uint128_t const * A, unsigned nRowsA,
  ...
  template <>
  void
  MM4RmultAddBlock<uint128_t,10>( uint128_t const * A, unsigned nRowsA,
                                  uint128_t const * B_Tables,
                                  uint128_t       * C ) {
  ...
  template <>
  void
  MM4RmultRightBlock<uint128_t,10>( uint128_t       * A, unsigned nRowsA,
                                    uint128_t const * B_Tables ) {
...
// EOF GF2toolkit_4RussianMult10bit.cc
